...
void GPIO_INIT(void){		//GPIO初始化
	...
	RCC_APB2PeriphClockCmd( RCC_APB2Periph_GPIOB, ENABLE );		//打开GPIOF~B
...
void LCD_INIT(void){	//初始化
	...
	GPIO_Write( GPIOF, 0x0000 );		//初始化GPIOF和GOIOB的值为0x0000，可以不用，个人习惯
...
void ReadBusy(void){		//读忙函数，读忙之前记得更改引脚的工作方式！！！因为STM32的IO不是准双向IO
	...
	p.GPIO_Pin = GPIO_Pin_7;		//选定GPIOF的第七Pin
	...
	while( GPIO_ReadInputDataBit( GPIOF, GPIO_Pin_7 ) );	//读第七Pin状态，如果一直为1则循环等待
	GPIO_ResetBits( GPIOB, EN );//使能关
		
	p.GPIO_Pin = GPIO_Pin_0|GPIO_Pin_1|GPIO_Pin_2|
				  GPIO_Pin_3|GPIO_Pin_4|GPIO_Pin_5|
				  GPIO_Pin_6|GPIO_Pin_7;		//使GPIOF的状态还原成推挽模式
